...
[
    ...
    { name: 'Incorrect Password Login', in_user: std_user, in_pass: "WRONG_PASS", err: 'Epic sadface: Username and password do not match any user in this service' },
    { name: 'Incorrect Username Login', in_user: 'WRONG_USER', in_pass: password, err: 'Epic sadface: Username and password do not match any user in this service' }
...
test('Successful standard login', async ({ webApp }) => {
    ...
    await webApp.locator('input#user-name').fill(std_user);
    await webApp.locator('input#password').fill(password);
...
test('Logout', async ({ userStdLoggedIn }) => {   
    // Confirm user has been successfully logged in
    const isloggedin = await userStdLoggedIn.locator('[data-test="title"]').textContent()
    expect(isloggedin).toContain("Products")

    // Click the burger navigation menu button
    await userStdLoggedIn.locator('#react-burger-menu-btn').click()
    // Click outlog
    await userStdLoggedIn.locator('.bm-item-list a#logout_sidebar_link').click()
    expect(userStdLoggedIn.locator('input#login-button'))
});